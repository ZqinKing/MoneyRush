...
YAHOO_MOEX_PAGE_HTML = """
<html>
  <head><title>MOEX Russia Index (IMOEX.ME) Charts, Data &amp; News - Yahoo Finance</title></head>
  <body>
    <section data-testid="quote-price">
      <span data-testid="qsp-price">2,842.11 </span>
      <span data-testid="qsp-price-change">+18.45 </span>
      <span data-testid="qsp-price-change-percent">(+0.65%) </span>
    </section>
  </body>
</html>
"""
...
def test_global_markets_fallback_parses_moex_yahoo_quote_page() -> None:
    snapshot = parse_yahoo_quote_page("moex_russia", YAHOO_MOEX_PAGE_HTML, now=NOW)
    ...
    assert payload["symbol"] == "IMOEX.ME"
    assert payload["providerSymbol"] == "IMOEX.ME"
    assert payload["source"] == "yahoo-finance-quote-page-moex-russia"
    assert payload["price"] == 2842.11
    assert payload["change"] == 18.45
    assert payload["changePercent"] == 0.65
...
def test_global_markets_client_uses_nasdaq_page_when_quote_api_is_rate_limited(monkeypatch) -> None:
    ...
    assert moex["providerSymbol"] == "IMOEX.ME"
    assert moex["source"] == "yahoo-finance-quote-page-moex-russia"
